...
# Function to list hosts from SSH config and connect using fzf
ssh_select() {
  ...
  if ! command -v fzf &> /dev/null; then
    ...
    echo "  apt-get install fzf # on Debian/Ubuntu"
    return 1
  fi
  ...
  # Parse SSH config file to extract host names, excluding wildcards and patterns
  local selected_host=$(grep -i "^Host " ~/.ssh/config | awk '{print $2}' | grep -v "[*?]" | fzf --height 40% --reverse --header="Select an SSH host")
...
alias just=ninja
alias aichat="ollama run mistral"

export VCA_PRUNE_ENGINES=false
...
# pnpm
export PNPM_HOME="/home/hebaishi/.local/share/pnpm"
case ":$PATH:" in
  *":$PNPM_HOME:"*) ;;
